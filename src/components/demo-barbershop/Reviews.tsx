        ...
        <div className="mt-14 grid grid-cols-1 gap-5 sm:grid-cols-2">
          {reviews.map((r) => (
            <figure
              key={r.name}
              data-reveal
              className="card-hover rounded-xl border border-[var(--line)] bg-[var(--charcoal)] p-6"
            >
              <Stars rating={r.rating} />
              <blockquote className="mt-4 text-[0.95rem] leading-relaxed text-ivory-soft">
                “{r.text}”
              </blockquote>
              <figcaption className="mt-4 text-sm font-semibold text-ivory">{r.name}</figcaption>
            </figure>
          ))}
        </div>
      </div>
